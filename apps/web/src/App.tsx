...
function HeaderSearch({ user }: { user: AuthUser | null }) {
  ...
  const can = useCallback((permission: string) => user?.role === "SUPER_ADMIN" || !!user?.permissions.includes(permission), [user]);
  ...
  useEffect(() => {
    if (normalized.length < 2) {
      const resetTimer = window.setTimeout(() => { setResults([]); setLoading(false); }, 0);
  ...
  return <div className={cn(`header-search ${open ? "search-open" : ""}`)} ref={searchRef}>
    ...
    {query ? <button type="button" className={cn("header-search-clear")} aria-label="Clear search" onClick={() => { setQuery(""); setResults([]); inputRef.current?.focus(); }}><X size={15} /></button> : <kbd>Ctrl K</kbd>}
    {open && normalized.length >= 2 && <div className={cn("header-search-results")} role="region" aria-label="Search results">
      <div className={cn("header-search-caption")}>{loading ? "Searching workspace…" : results.length ? `${results.length} matching records` : "No matching records"}</div>
      {results.map((item) => <Link className={cn("header-search-result")} href={item.href} key={`${item.kind}-${item.id}`} onClick={() => { setOpen(false); setQuery(""); }}><span className={cn("header-search-kind")}>{item.kind}</span><span className={cn("header-search-result-copy")}><strong>{item.title}</strong><small>{item.detail}</small></span><ArrowRight size={15} /></Link>)}
    </div>}
  </div>;
}
    ...
    <div className={cn(`app-shell ${sidebarCollapsed ? "sidebar-collapsed" : ""}`)}>
      <aside className={cn(`sidebar ${mobileOpen ? "sidebar-open" : ""}`)}>
        ...
        <div className={cn("tenant-switcher")}>
          <div className={cn("tenant-avatar")}>AT</div>
          <div>
            <strong>{user?.agencyName ?? "A-One Tours"}</strong>
            <span>Organization</span>
          </div>
          <ChevronDown size={20} />
          <button className={cn("sidebar-collapse-toggle")} type="button" aria-label={sidebarCollapsed ? "Expand sidebar" : "Collapse sidebar"} title={sidebarCollapsed ? "Expand sidebar" : "Collapse sidebar"} onClick={() => setSidebarCollapsed((collapsed) => !collapsed)}>
        ...
        <nav className={cn("navigation")} aria-label="Primary navigation">
          {navGroups.map((group) => (
            <div className={cn("nav-group")} key={group.label}>
              {group.label && <p className={cn("nav-label")}>{group.label}</p>}
              {group.items
                .filter(
                  (item) =>
                    !item.permission ||
      ...
      <main className={cn("main-area")}>
        <header className={cn("topbar")}>
          ...
          <div className={cn("topbar-page-meta")} aria-live="polite">
            ...
            {workspaceHeading?.description && <p className={cn("topbar-subtitle")}>{workspaceHeading.description}</p>}
          </div>
          {workspaceHeading?.action && <div className={cn("topbar-page-action")}>{workspaceHeading.action}</div>}
          <div className={cn("topbar-actions")}>
            <HeaderSearch user={user} />
            <Link className={cn("icon-button notification-button")} href="/dashboard/notifications" aria-label="Notifications" title="Notifications">
              <Bell size={19} />
            </Link>
            <details className={cn("profile-menu")}>
              <summary className={cn("profile profile-compact")}>
              <span className={cn("profile-symbol")}><UserRound size={17} /></span>
              ...
              {user && <div className={cn("profile-dropdown")}>
                <div className={cn("profile-dropdown-identity")}><strong>{user.firstName} {user.lastName}</strong><span>{user.email}</span><span>{user.agencyName ?? "A-One Tours"}</span></div>
                <Link href="/dashboard/profile">Profile</Link>
                <Link href="/dashboard/settings">Settings</Link>
                <button type="button" onClick={() => { if (window.confirm("Are you sure you want to log out?")) void logout(); }}><LogOut size={15} /> Log out</button>
              </div>}
            </details>
          </div>
        </header>
  ...
  return (
    <div className={cn("login-page")}>
      ...
      <Card className={cn("login-card")}>
        ...
        <Button>Authentication coming soon <ArrowRight size={15} /></Button>
      </Card>
    </div>
  );
}
