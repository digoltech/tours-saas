...
export default function InvitationPage() {
  ...
  return <AuthLayout><div className={cn("auth-card-stack auth-card-stack-narrow")}><div className={cn("auth-heading")}><p className={cn("eyebrow")}>Team invitation</p><h2>{loading ? "Loading your invitation…" : invite ? `Welcome, ${invite.firstName}.` : "Invitation unavailable"}</h2><p>{invite ? `You have been invited to join ${invite.agencyName}. Create a password to activate your account.` : error}</p></div><Card className={cn("login-card invitation-card")}>{loading ? <div className={cn("auth-loading-state")}><span className={cn("auth-spinner")} />Loading your invitation</div> : invite ? <form onSubmit={submit}><Input label="Email address" id="inviteEmail" value={invite.email} readOnly /><PasswordInput label="Create password" id="invitePassword" value={password} onChange={(event) => setPassword(event.target.value)} autoComplete="new-password" /><PasswordInput label="Confirm password" id="inviteConfirmation" value={confirmation} onChange={(event) => setConfirmation(event.target.value)} autoComplete="new-password" /><p className={cn("password-hint")}>Use at least 8 characters.</p>{error && <p className={cn("form-error")} role="alert">{error}</p>}<Button type="submit" disabled={saving}>{saving ? "Activating account…" : "Join workspace"}</Button></form> : <Link className={cn("button button-secondary")} href="/login">Back to sign in</Link>}</Card></div></AuthLayout>;
}
