...
const methods: FinanceMethodValue[] = [
  "CASH",
  "BANK_TRANSFER",
  "CARD",
  "UPI",
...
export function FinanceWorkspace() {
  ...
  const [booking, setBooking] = useState<Awaited<
    ReturnType<typeof getBookingFinanceByPnr>
  > | null>(null);
  ...
  useEffect(() => {
    void getTrips({ limit: "100" })
      .then((page) => setTrips(page.data as TripOption[]))
      .catch(() => setTrips([]));
  }, []);
  ...
  async function findBooking() {
    ...
    setMessage("");
    try {
      setBooking(await getBookingFinanceByPnr(pnr.trim()));
      setFormAmount("");
    } catch (cause) {
      setBooking(null);
  ...
  async function saveSettings() {
    ...
    const tiers = settings.tiers.filter(
    ...
    await act(
      () =>
        saveFinanceSettings({
          ...
          gstRate: gst,
          gstAfterDiscount: settings.gstAfterDiscount,
          commissionType: settings.commissionType,
          commissionValue: commission,
          tiers: tiers.map((tier) => ({
            hoursBeforeDeparture: Number(tier.hoursBeforeDeparture),
            feePercent: Number(tier.feePercent),
          })),
        }),
  ...
  const stats = [
    {
      label: "Collected",
      value: money(reports?.totals.revenue ?? 0),
    ...
    {
      label: "Refunded",
      value: money(reports?.totals.refunds ?? 0),
      detail: "Refunds recorded",
      icon: ArrowDownRight,
      color: "rose",
    },
    {
      label: "Bookings",
      value: reports?.totals.bookings ?? "—",
    ...
    {
      label: "Cancellations",
      value: reports?.totals.cancellations ?? "—",
      detail: "Bookings cancelled",
      icon: Activity,
      color: "amber",
    },
    {
      label: "Occupancy",
      value: `${reports?.totals.occupancyPercent ?? 0}%`,
      detail: "Seats sold in selected trips",
      icon: Users,
      color: "violet",
    },
    {
      label: "Commission",
      value: money(reports?.totals.commission ?? 0),
      detail: "Net agent commission",
      icon: Banknote,
      color: "teal",
    },
  ];
    ...
    <>
      ...
      <div className={cn("finance-toolbar-card")}>
        ...
        <div className={cn("finance-filters")}>
          ...
          <button
            className={cn("finance-clear-filters")}
            type="button"
      ...
      <div className={cn("dashboard-stats finance-stat-grid")}>
        {stats.map(({ label, value, detail, icon: Icon, color }) => (
          <Card className={cn("finance-stat-card")} key={label}>
            <div className={cn(`finance-stat-icon ${color}`)}>
              <Icon size={18} />
            </div>
            <div className={cn("finance-stat-copy")}>
              <span>{label}</span>
      ...
      <div className={cn("finance-chart-grid")} id="finance-overview">
        <Card className={cn("finance-panel finance-revenue-panel")}>
          ...
          {dailySales.length ? (
            <div className={cn("finance-line-chart-wrap")}>
              <svg
                className={cn("finance-line-chart")}
                viewBox="0 0 720 230"
                role="img"
                aria-label="Daily booking value trend"
              >
                {[0, 1, 2, 3].map((line) => (
                  <line
                    key={line}
                    ...
                    y1={24 + line * 54}
                    y2={24 + line * 54}
                    className={cn("finance-chart-gridline")}
                  />
                ))}
                {(() => {
                  const points = dailySales
                    .map(([, value], index) => {
                      const x =
                        dailySales.length === 1
                          ? 373
                          : 48 + index * (648 / (dailySales.length - 1));
                  ...
                  const fill =
                    first && last
                      ? `${first[0]},190 ${points} ${last[0]},190`
                      : points;
                  return (
                    <>
                      <polygon
                        points={fill}
                        className={cn("finance-chart-area")}
                      />
                      <polyline
                        points={points}
                        className={cn("finance-chart-line")}
                      />
                      {dailySales.map(([day, value], index) => {
                        const x =
                          dailySales.length === 1
                            ? 373
                            : 48 + index * (648 / (dailySales.length - 1));
                        ...
                        return (
                          <circle
                            ...
                            cy={y}
                            r="4.5"
                            className={cn("finance-chart-dot")}
                          >
                            <title>
              ...
              <div className={cn("finance-chart-labels")}>
                {dailySales
                  .filter(
                    (_, index) =>
                      index === 0 ||
                      index === dailySales.length - 1 ||
                      index % Math.max(1, Math.ceil(dailySales.length / 5)) ===
                        0,
                  )
                  .map(([day]) => (
                    <span key={day}>
        ...
        <Card className={cn("finance-panel finance-mix-panel")}>
            ...
            <strong>
              {money(
                Number(reports?.totals.revenue ?? 0) -
                  Number(reports?.totals.refunds ?? 0),
              )}
            </strong>
          </div>
          {(
            [
              ...
              [
                "Commission",
                Number(reports?.totals.commission ?? 0),
                "commission",
              ],
            ] as const
            ...
            return (
              <div className={cn("finance-mix-row")} key={label}>
                <div>
                  <span>{label}</span>
                  <strong>{money(value)}</strong>
                </div>
                <div className={cn("finance-mix-track")}>
                  <i
                    className={cn(color)}
                    style={{
          ...
          <div className={cn("finance-cancellation-ratio")}>
            <div
            ...
            <div className={cn("finance-entry-grid")}>
              <label>
                ...
                <input
                  ...
                  onChange={(event) =>
                    setSettings({ ...settings, gstRate: event.target.value })
                  }
                />
              </label>
              <label>
                ...
                <select
                  ...
                  onChange={(event) =>
                    setSettings({
                      ...settings,
                      gstAfterDiscount: event.target.value === "true",
                    })
                  }
                >
                  ...
                  onChange={(event) =>
                    setSettings({
                      ...
                      commissionType: event.target
                        .value as FinanceSettings["commissionType"],
                    })
                  }
                >
              ...
              <Button
                ...
                onClick={() =>
                  setSettings({
                    ...
                    tiers: [
                      ...settings.tiers,
                      { hoursBeforeDeparture: "24", feePercent: "0" },
                    ],
                  })
                }
              >
                ...
                <label>
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      setSettings({
                        ...settings,
                        tiers: settings.tiers.map((item, i) =>
                          i === index
                            ? {
                                ...item,
                                hoursBeforeDeparture: event.target.value,
                              }
                            : item,
                        ),
                      })
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      setSettings({
                        ...
                        tiers: settings.tiers.map((item, i) =>
                          i === index
                            ? { ...item, feePercent: event.target.value }
                            : item,
                        ),
                      })
                    }
