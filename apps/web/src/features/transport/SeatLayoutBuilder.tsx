...
export function SeatLayoutBuilder() {
  ...
  const layoutRows = useMemo(
    () => Math.max(rows, Math.ceil(seatCount / columns)),
    [rows, seatCount, columns],
  );
  useEffect(() => {
    if (!busId) return;
    getSeatLayout(busId)
  ...
  function save() {
    if (!busId) return;
    setError("");
    saveSeatLayout(busId, { rows, columns, disabledSeats: disabled })
      .then(() => setSaved(true))
      .catch((cause) =>
        setError(
  ...
  return (
    <>
      <PageHeader
        ...
        description="Arrange available seats for a bus and mark driver or non passenger positions as unavailable."
        action={
      ...
      <Card className="management-form">
        ...
        <p className="muted">
          Layout: {layoutRows} rows × {columns} columns · {seatCount} seat
          positions. Select a seat to mark it unavailable.
        </p>
      </Card>
      {!busId ? (
        <Card>
        ...
        <Card className="seat-layout-card">
          <div className="card-heading">
            <div>
              <p className="eyebrow">Front of bus</p>
            ...
            {Array.from({ length: seatCount }, (_, index) => {
              ...
              return (
                <button
                  key={name}
                  type="button"
                  className={`seat-button ${unavailable ? "seat-unavailable" : ""}`}
                  aria-pressed={unavailable}
                  aria-label={`${name}, ${unavailable ? "unavailable" : "available"}`}
                  onClick={() => {
                    setDisabled((current) =>
                      unavailable
